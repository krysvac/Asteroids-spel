...
class CSource
{
    ...
    public function __construct($options = array())
    {
        $default = array(
            ...
            'add_ignore' => null,
             // add array with additional filenames to ignore
            'secure_dir' => '.',
             // Only display files below this directory
            'base_dir' => '.',
             // Which directory to start look in, defaults to current working directory of the actual script.
            'query_dir' => isset($_GET['dir']) ? strip_tags(trim($_GET['dir'])) : null,
             // Selected directory as ?dir=xxx
            'query_file' => isset($_GET['file']) ? strip_tags(trim($_GET['file'])) : null,
             // Selected directory as ?dir=xxx
            'query_path' => isset($_GET['path']) ? strip_tags(trim($_GET['path'])) : null,
             // Selected directory as ?dir=xxx
    ...
    public function GetFileContent()
    {
        ...
        if (in_array($this->extension, $this->validImageExtensions)) {
            ...
            $baseDir = !empty($this->options['base_dir']) ? rtrim($this->options['base_dir'], '/') . '/' : null;
